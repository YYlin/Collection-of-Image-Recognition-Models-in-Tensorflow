...
def bottleneck(net, channel, holes=1, c_name='pretrain', stride=1,
    ...
    with tf.variable_scope('bottleneck_v2', reuse=tf.AUTO_REUSE):
        ...
        # batch normalization
        net = tf.layers.batch_normalization(inputs=net, axis=-1, name='preact')
        net = tf.nn.relu(net)
...
def block(net, name, unit, channel):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        for i in range(unit):
            with tf.variable_scope('unit_' + str(i + 1), reuse=tf.AUTO_REUSE):
                ...
                if i == 0:
                    if name != 'block1':
                        net, shortcut = bottleneck(net, channel,
                                                   stride=2,
                                                   shortcut_conv=True)
                    else:
                        net, shortcut = bottleneck(net, channel,
...
def resnet(input_, resnet_v2, classes):
    ...
    with tf.variable_scope(resnet_v2, reuse=tf.AUTO_REUSE):
        ...
        net = tf.layers.batch_normalization(inputs=net, axis=-1, name='postnorm')
        net = tf.nn.relu(net)
